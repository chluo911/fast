...
def handle_node(G: Graph, node_id, extra=None) -> NodeHandleResult:
    ...
    if G.function_time_limit and G.func_start_time is not None \
        ...
        console.log("Function time limit reached. Halting...")
        console.log(node_id)
    if G.finished or G.time_limit_reached:
        return NodeHandleResult()
...
def simurun_block(G, ast_node, parent_scope=None, branches=None,
    ...
    for stmt in stmts:
        ...
        #if G.pause_at_next_stmt == G.cur_scope: # debugger
        #    G.pause_at_next_node = True
        #    G.pause_at_next_stmt = None
...
def build_rough_call_graph(G: Graph, entry_node):
    ...
    def dummy_handle_node(G: Graph, ast_node):
        ...
        # handle DSL
        comments = G.get_node_attr(ast_node).get('doccomment')
        if comments:
            for comment in comments.split('\\n'):
                if comment == 'OPG: entry':
                    func_name = get_func_name(G, ast_node)
                    G.func_entry_point = func_name
                elif comment == 'OPG: sink':
                    func_name = get_func_name(G, ast_node)
                    signature_decls += func_dict[func_name]
                elif comment == 'OPG: pause':
                    G.breakpoints.add(ast_node)
